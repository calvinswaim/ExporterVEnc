...
#pragma region Group: Basic Video Encoder Settings
	...
	/* Set min value to crf and qp for lossless option */
	exParamValues profileValue, tempValue;
	instRec->exportParamSuite->GetParamValue(exID, groupIndex, X264_Profile, &profileValue);
	csSDK_int32 rangeMin = profileValue.value.intValue == (int)X264::Profile::High444 ? 0 : 1;
	instRec->exportParamSuite->GetParamValue(exID, groupIndex, X264_Strategy_CRF, &tempValue);
	tempValue.rangeMin.intValue = rangeMin;
	instRec->exportParamSuite->ChangeParam(exID, groupIndex, X264_Strategy_CRF, &tempValue);
	instRec->exportParamSuite->GetParamValue(exID, groupIndex, X264_Strategy_QP, &tempValue);
	tempValue.rangeMin.intValue = rangeMin;
	instRec->exportParamSuite->ChangeParam(exID, groupIndex, X264_Strategy_QP, &tempValue);
...
prMALError exValidateParamChanged(exportStdParms *stdParmsP, exParamChangedRec *validateParamChangedRecP)
{
	...
	if (instRec->exportParamSuite)
	{
		...
		else if (strcmp(validateParamChangedRecP->changedParamIdentifier, X264_Profile) == 0)
		{
			/* Lossless is only supported in High444 profile */
			csSDK_int32 rangeMin = changedValue.value.intValue == (int)X264::Profile::High444 ? 0 : 1;
			instRec->exportParamSuite->GetParamValue(exID, validateParamChangedRecP->multiGroupIndex, X264_Strategy_CRF, &tempValue);
			tempValue.rangeMin.intValue = rangeMin;
			instRec->exportParamSuite->ChangeParam(exID, validateParamChangedRecP->multiGroupIndex, X264_Strategy_CRF, &tempValue);
			instRec->exportParamSuite->GetParamValue(exID, validateParamChangedRecP->multiGroupIndex, X264_Strategy_QP, &tempValue);
