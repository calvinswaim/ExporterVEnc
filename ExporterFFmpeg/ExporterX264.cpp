...
prMALError exExport(exportStdParms *stdParmsP, exDoExportRec *exportInfoP)
{
	...
	/* Export video params */
	exParamValues videoCodec, videoWidth, videoHeight, pixelAspectRatio, ticksPerFrame, fieldType, audioCodec, channelType, audioSampleRate, audioBitrate, x264Profile;
	...
	instRec->exportParamSuite->GetParamValue(exID, 0, X264_Profile, &x264Profile);
	...
	if (x264Profile.value.intValue == (csSDK_int32)X264::Profile::High422)
	{
		pixelFormat = AV_PIX_FMT_YUV422P;
	}
	else if (x264Profile.value.intValue == (csSDK_int32)X264::Profile::High444)
	{
	...
	instRec->timeSuite->GetTicksPerAudioSample(PLUGIN_AUDIO_SAMPLE_RATE, &ticksPerSample);
	instRec->sequenceAudioSuite->MakeAudioRenderer(exID, exportInfoP->startTime, (PrAudioChannelType)channelType.value.intValue, kPrAudioSampleType_32BitFloat, PLUGIN_AUDIO_SAMPLE_RATE, &audioRenderID);
	...
	encoder->setAudioCodec(static_cast<AVCodecID>(audioCodec.value.intValue), channelLayout, audioBitrate.value.intValue * 1000, { 1, static_cast<int>(audioSampleRate.value.floatValue) });
	...
	csSDK_int32 frameSize = PLUGIN_AUDIO_FRAME_SIZE;

	csSDK_int32 samplesRequestedL;
	PrTime ticksPerAudioFrame = ticksPerSample * frameSize;
	...
	while (videoTime < exportInfoP->endTime)
	{
		if (videoTime >= nextVideoFrame)
		{
			...
			result = instRec->sequenceRenderSuite->RenderVideoFrame(videoRenderID, nextVideoFrame, &renderParms, kRenderCacheType_None, &renderResult);
		...
		if (videoTime >= nextAudioFrame)
		{
			...
			/* Let ffmpeg encode the frame */
			encoder->writeAudioFrame((const uint8_t**)audioBuffer, samplesRequestedL);
